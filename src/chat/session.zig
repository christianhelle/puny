...
fn savePlanningPrd(ctx: *ChatLoopContext) !void {
    ...
    {
        const html = try mdToHtml(ctx.messages_arena.allocator(), text);
        const file = cwd.createFile(ctx.io, ctx.session.html_path, .{}) catch |err| {
            try ctx.stdout_writer.print("\n{s}Warning: could not save PRD HTML to {s}: {s}{s}\n", .{ ansi.dim, ctx.session.html_path, @errorName(err), ansi.reset });
            try ctx.stdout_writer.flush();
            return;
        };
        defer file.close(ctx.io);
        file.writeStreamingAll(ctx.io, html) catch |err| {
            try ctx.stdout_writer.print("\n{s}Warning: could not write PRD HTML: {s}{s}\n", .{ ansi.dim, @errorName(err), ansi.reset });
            try ctx.stdout_writer.flush();
            return;
        };
    }
    try ctx.stdout_writer.print("\n{s}PRD saved to {s} and {s}{s}\n", .{ ansi.dim, ctx.session.prd_path, ctx.session.html_path, ansi.reset });
    try ctx.stdout_writer.flush();
}

fn mdToHtml(arena: std.mem.Allocator, md: []const u8) ![]const u8 {
    var html: std.ArrayList(u8) = .empty;
    errdefer html.deinit(arena);
    ...
    try html.appendSlice(arena,
        ...
        \\<title>Plan</title>
        \\<style>
        \\  body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 2em; line-height: 1.6; color: #1a1a1a; }
    ...
    while (lines.next()) |raw_line| {
        ...
        if (std.mem.startsWith(u8, line, "```")) {
            if (in_code_block) {
                try html.appendSlice(arena, "</code></pre>\n");
                in_code_block = false;
            } else {
                if (in_list) { try html.appendSlice(arena, "</ul>\n"); in_list = false; }
                if (in_ordered_list) { try html.appendSlice(arena, "</ol>\n"); in_ordered_list = false; }
                try html.appendSlice(arena, "<pre><code>");
                in_code_block = true;
            }
            continue;
        }
        ...
        if (std.mem.startsWith(u8, line, "### ")) {
            if (in_list) { try html.appendSlice(arena, "</ul>\n"); in_list = false; }
            if (in_ordered_list) { try html.appendSlice(arena, "</ol>\n"); in_ordered_list = false; }
            try html.appendSlice(arena, "<h3>");
            try html.appendSlice(arena, inlineMdToHtml(arena, line[4..]));
            try html.appendSlice(arena, "</h3>\n");
            continue;
        }
        ...
        if (std.mem.startsWith(u8, line, "## ")) {
            if (in_list) { try html.appendSlice(arena, "</ul>\n"); in_list = false; }
            if (in_ordered_list) { try html.appendSlice(arena, "</ol>\n"); in_ordered_list = false; }
            try html.appendSlice(arena, "<h2>");
            try html.appendSlice(arena, inlineMdToHtml(arena, line[3..]));
            try html.appendSlice(arena, "</h2>\n");
            continue;
        }
        ...
        if (std.mem.startsWith(u8, line, "# ")) {
            ...
            try html.appendSlice(arena, inlineMdToHtml(arena, line[2..]));
            try html.appendSlice(arena, "</h1>\n");
            continue;
        }
        ...
        if (std.mem.startsWith(u8, line, "> ")) {
            if (in_list) { try html.appendSlice(arena, "</ul>\n"); in_list = false; }
            if (in_ordered_list) { try html.appendSlice(arena, "</ol>\n"); in_ordered_list = false; }
            try html.appendSlice(arena, "<blockquote><p>");
            try html.appendSlice(arena, inlineMdToHtml(arena, line[2..]));
            try html.appendSlice(arena, "</p></blockquote>\n");
            continue;
        }
        ...
        if (line.len >= 3 and std.mem.allEqual(u8, line, '-')) {
            if (in_list) { try html.appendSlice(arena, "</ul>\n"); in_list = false; }
            if (in_ordered_list) { try html.appendSlice(arena, "</ol>\n"); in_ordered_list = false; }
            try html.appendSlice(arena, "<hr>\n");
            continue;
        }

        if (in_list) { try html.appendSlice(arena, "</ul>\n"); in_list = false; }
        if (in_ordered_list) { try html.appendSlice(arena, "</ol>\n"); in_ordered_list = false; }
        try html.appendSlice(arena, "<p>");
        try html.appendSlice(arena, inlineMdToHtml(arena, line));
        try html.appendSlice(arena, "</p>\n");
    }

    if (in_code_block) try html.appendSlice(arena, "</code></pre>\n");
    if (in_list) try html.appendSlice(arena, "</ul>\n");
    if (in_ordered_list) try html.appendSlice(arena, "</ol>\n");

    try html.appendSlice(arena, "</body>\n</html>\n");
    return html.toOwnedSlice(arena);
}

fn inlineMdToHtml(arena: std.mem.Allocator, text: []const u8) []const u8 {
    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(arena);
    var i: usize = 0;
    while (i < text.len) {
        if (text[i] == '\\' and i + 1 < text.len) {
            out.append(arena, text[i + 1]) catch break;
            i += 2;
            continue;
        }
        if (std.mem.startsWith(u8, text[i..], "**")) {
            const end = std.mem.indexOf(u8, text[i + 2 ..], "**") orelse {
                out.appendSlice(arena, text[i..]) catch break;
                break;
            };
            out.appendSlice(arena, "<strong>") catch break;
            out.appendSlice(arena, inlineMdToHtml(arena, text[i + 2 .. i + 2 + end])) catch break;
            out.appendSlice(arena, "</strong>") catch break;
            i += 4 + end;
            continue;
        }
        if (text[i] == '*' and i + 1 < text.len and text[i + 1] != '*') {
            const end = std.mem.indexOfScalar(u8, text[i + 1 ..], '*') orelse {
                out.appendSlice(arena, text[i..]) catch break;
                break;
            };
            out.appendSlice(arena, "<em>") catch break;
            out.appendSlice(arena, inlineMdToHtml(arena, text[i + 1 .. i + 1 + end])) catch break;
            out.appendSlice(arena, "</em>") catch break;
            i += 2 + end;
            continue;
        }
        if (text[i] == '`') {
            ...
            out.appendSlice(arena, "<code>") catch break;
            out.appendSlice(arena, escapeHtml(arena, text[i + 1 .. i + 1 + end])) catch break;
            out.appendSlice(arena, "</code>") catch break;
            i += 2 + end;
            continue;
        }
        if (text[i] == '<') {
            out.appendSlice(arena, "&lt;") catch break;
            i += 1;
            continue;
        }
        if (text[i] == '>') {
        ...
        out.append(arena, text[i]) catch break;
        i += 1;
    }
    return out.toOwnedSlice(arena) catch text;
}

fn escapeHtml(arena: std.mem.Allocator, text: []const u8) []const u8 {
    ...
    for (text) |c| {
        switch (c) {
            ...
            '"' => out.appendSlice(arena, "&quot;") catch break,
            '\'' => out.appendSlice(arena, "&#39;") catch break,
            else => out.append(arena, c) catch break,
        }
    }
    return out.toOwnedSlice(arena) catch text;
}
