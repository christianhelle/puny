...
pub fn main(init: std.process.Init) !void {
    ...
    var model_key = (try model_selection.select(&prov, configured_model, arena, io, skip_validation, cfg, init.environ_map)) orelse blk: {
        if (configured_model) |model_id| {
            try stdout_writer.print("Model '{s}' not found in running models. Showing picker.\n", .{model_id});
        }
        break :blk (try model_selection.select(&prov, null, arena, io, false, cfg, init.environ_map)) orelse {
    ...
    while (true) {
        ...
        switch (action) {
            ...
            .switch_model => |model_id| {
                ...
                if (try model_selection.switchModel(&prov, model_id, model_key, arena, io, model_skip_validation, stdout_writer, cfg, init.environ_map)) |new_key| {
                    model_key = new_key;
                }
                continue;
            },
