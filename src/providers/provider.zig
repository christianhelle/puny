...
/// intentionally routes LM Studio, Ollama, OpenCode Zen/Go via the OpenAI-compatible
...
test "Provider.listModels dispatches to the ollama provider" {
    const body =
        \\{"object":"list","data":[{"id":"qwen3:8b","object":"model","created":1757000000,"owned_by":"library"}]}
    ;
    const ctx = try startProviderTestServer(.ok, body);
    defer stopProviderTestServer(ctx);
    const url = try providerTestUrl(ctx);
    ...
    defer owned.deinit();
    try std.testing.expectEqualStrings("/v1/models", ctx.getRequestPath());
    try expectModelIds(&owned, &.{"qwen3:8b"});
}

test "Provider.chatStreaming dispatches to the ollama provider" {
    const body =
    ...
    switch (rec.events.items[0]) {
        .content => |content| try std.testing.expectEqualStrings("Hi from Ollama", content),
        else => return error.ExpectedContentEvent,
    }
}

test "Provider.chatStreaming preserves ollama HTTP error details" {
    const body = "{\"error\":{\"message\":\"model \\\"missing\\\" not found, try pulling it first\"}}";
    const ctx = try startProviderTestServer(.not_found, body);
    defer stopProviderTestServer(ctx);
    const url = try providerTestUrl(ctx);
    defer std.testing.allocator.free(url);
...
test "Provider.chatStreaming dispatches to the responses transport for gpt" {
    const body =
        "data: {\"type\":\"response.output_text.delta\",\"item_id\":\"msg_1\",\"output_index\":0,\"content_index\":0,\"delta\":\"Hello GPT\",\"sequence_number\":1,\"logprobs\":[]}\n\n" ++
        "data: {\"type\":\"response.completed\",\"sequence_number\":2,\"response\":{\"id\":\"resp_123\",\"object\":\"response\",\"status\":\"completed\",\"model\":\"gpt-5.5\",\"output\":[],\"usage\":{\"input_tokens\":1,\"output_tokens\":1,\"total_tokens\":2}}}\n\n" ++
        "data: [DONE]\n\n";
    const ctx = try startProviderTestServer(.ok, body);
    defer stopProviderTestServer(ctx);
    const url = try providerTestUrl(ctx);
    ...
    try std.testing.expectEqual(@as(usize, 3), rec.events.items.len);
    switch (rec.events.items[0]) {
        .content => |content| try std.testing.expectEqualStrings("Hello GPT", content),
        else => return error.ExpectedContentEvent,
    }
}

test "Provider.chatStreaming dispatches to the responses transport for grok" {
    const body =
        "data: {\"type\":\"response.output_text.delta\",\"item_id\":\"msg_1\",\"output_index\":0,\"content_index\":0,\"delta\":\"Hello Grok\",\"sequence_number\":1,\"logprobs\":[]}\n\n" ++
        "data: {\"type\":\"response.completed\",\"sequence_number\":2,\"response\":{\"id\":\"resp_123\",\"object\":\"response\",\"status\":\"completed\",\"model\":\"grok-4.5\",\"output\":[],\"usage\":{\"input_tokens\":1,\"output_tokens\":1,\"total_tokens\":2}}}\n\n" ++
        "data: [DONE]\n\n";
    const ctx = try startProviderTestServer(.ok, body);
    defer stopProviderTestServer(ctx);
    const url = try providerTestUrl(ctx);
    ...
    try std.testing.expectEqual(@as(usize, 3), rec.events.items.len);
    switch (rec.events.items[0]) {
        .content => |content| try std.testing.expectEqualStrings("Hello Grok", content),
        else => return error.ExpectedContentEvent,
    }
}
...
const StallErrorServer = struct {
    ...
    fn serve(self: *@This()) void {
        ...
        body.writer.writeAll("{\"error\":{\"message\":\"partial") catch return;
        body.flush() catch return;
        // Stall for 5s - the watcher must interrupt the blocked read promptly
        self.io.sleep(.{ .nanoseconds = 5000 * std.time.ns_per_ms }, .awake) catch {};
        body.writer.writeAll(" failure\"}}") catch return;
        body.flush() catch return;
    }
};

fn startStallErrorServer() !*StallErrorServer {
    const address: std.Io.net.IpAddress = .{ .ip4 = std.Io.net.Ip4Address.loopback(0) };
    const server = std.Io.net.IpAddress.listen(&address, std.testing.io, .{}) catch return error.ListenFailed;
    const ctx = try std.testing.allocator.create(StallErrorServer);
    errdefer std.testing.allocator.destroy(ctx);
    ctx.* = .{ .io = std.testing.io, .server = server };
    errdefer ctx.server.deinit(std.testing.io);
    ctx.thread = try std.Thread.spawn(.{}, StallErrorServer.serve, .{ctx});
    return ctx;
}

fn stopStallErrorServer(ctx: *StallErrorServer) void {
    ctx.server.deinit(std.testing.io);
    ctx.thread.join();
    std.testing.allocator.destroy(ctx);
}

fn stallErrorServerUrl(ctx: *StallErrorServer) ![]u8 {
    return std.fmt.allocPrint(std.testing.allocator, "http://127.0.0.1:{d}", .{ctx.server.socket.address.getPort()});
}

test "chatStreaming aborts promptly when server stalls draining a non-success body after cancel" {
    const ctx = try startStallErrorServer();
    defer stopStallErrorServer(ctx);
    const url = try stallErrorServerUrl(ctx);
    defer std.testing.allocator.free(url);
    ...
    try std.testing.expectError(error.Canceled, prov.chatStreaming(request, rec.callback()));
    const elapsed = start.untilNow(std.testing.io, .awake).nanoseconds;
    // Must abort well before the 5s stall completes - the cancellation watcher
    // must also interrupt a blocked read while draining a non-success body.
    try std.testing.expect(elapsed < 2000 * std.time.ns_per_ms);
}
