...
const AnthropicGarbageServer = struct {
    io: std.Io,
    server: std.Io.net.Server,
    thread: std.Thread = undefined,
...
fn startAnthropicGarbageServer() !*AnthropicGarbageServer {
    const address: std.Io.net.IpAddress = .{ .ip4 = std.Io.net.Ip4Address.loopback(0) };
    const server = std.Io.net.IpAddress.listen(&address, std.testing.io, .{}) catch return error.ListenFailed;
    const ctx = try std.testing.allocator.create(AnthropicGarbageServer);
    errdefer std.testing.allocator.destroy(ctx);
    ctx.* = .{ .io = std.testing.io, .server = server };
    ctx.thread = try std.Thread.spawn(.{}, AnthropicGarbageServer.serve, .{ctx});
    return ctx;
}

fn stopAnthropicGarbageServer(ctx: *AnthropicGarbageServer) void {
    ctx.thread.join();
    ctx.server.deinit(std.testing.io);
    std.testing.allocator.destroy(ctx);
}

test "chatStreamingAnthropic reports response head failures to the http observer" {
    const ctx = try startAnthropicGarbageServer();
    defer stopAnthropicGarbageServer(ctx);
...
test "chatStreamingAnthropic returns Canceled when the stream is cancelled" {
    const body =
